...
const getAllPacientes = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, data: pacientesData, total: pacientesData.length });
...
const createPaciente = async (req, res) => {
    try {
        ...
        const result = await db.transaction(async (tx) => {
            // 1. Crear el usuario - CORREGIR AQUÍ
            const [nuevoUsuario] = await tx.insert(usuarios).values({
                ...
                activo: true
            }).returning({ idUsuario: usuarios.idUsuario });

            // ... el resto del código se mantiene igual ...
            let usuarioId = nuevoUsuario && (nuevoUsuario.idUsuario ?? nuevoUsuario.id_usuario ?? nuevoUsuario.id);
            if (!usuarioId) {
                const encontrado = await tx.select({ idUsuario: usuarios.idUsuario })
                    .from(usuarios)
                    .where(eq(usuarios.numeroDocumento, numeroDocumento))
                    .limit(1);
                if (encontrado && encontrado.length > 0) usuarioId = encontrado[0].idUsuario;
            }

            if (!usuarioId) throw new Error('No se pudo recuperar idUsuario después de insertar usuario');
            ...
            const [nuevoPaciente] = await tx.insert(pacientes).values({
                ...
                estadoCivil,
                ocupacion
            }).returning({ idPaciente: pacientes.idPaciente });

            const pacienteId = nuevoPaciente && (nuevoPaciente.idPaciente ?? nuevoPaciente.id_paciente ?? nuevoPaciente.id);
            if (!pacienteId) throw new Error('No se pudo recuperar idPaciente después de insertar paciente');

            // Obtener el paciente completo
            const pacienteCompleto = await tx.select({
                idPaciente: pacientes.idPaciente,
                tipoSangre: pacientes.tipoSangre,
                alergias: pacientes.alergias,
                condicionesMedicas: pacientes.condicionesMedicas,
            ...
            .from(pacientes)
            .innerJoin(usuarios, eq(pacientes.idUsuario, usuarios.idUsuario))
            .where(eq(pacientes.idPaciente, pacienteId))
            .limit(1);

            return { usuario: nuevoUsuario, paciente: pacienteCompleto[0] };
        });
...
const updatePaciente = async (req, res) => {
    try {
        ...
        const pacienteExistente = await db
            .select({ idPaciente: pacientes.idPaciente, idUsuario: pacientes.idUsuario })
            .from(pacientes)
            .where(eq(pacientes.idPaciente, parseInt(id)))
            .limit(1);
        ...
        const pacienteActualizado = await db
            .select({
                idPaciente: pacientes.idPaciente,
                tipoSangre: pacientes.tipoSangre,
                alergias: pacientes.alergias,
        ...
        res.status(200).json({ success: true, message: "Paciente actualizado exitosamente", data: pacienteActualizado[0] });
...
const deletePaciente = async (req, res) => {
    try {
        ...
        const { idUsuario } = pacienteExistente[0];
        await db.update(usuarios)
            .set({ activo: false, updatedAt: new Date() })
            .where(eq(usuarios.idUsuario, idUsuario));

        res.status(200).json({ success: true, message: "Paciente desactivado exitosamente", idUsuario });
